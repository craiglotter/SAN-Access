...
Public Class Main_Screen
    ...
    Private currentpasswordmasked As String = ""
    ...
    Private Sub Activity_Handler(ByVal message As String, Optional ByVal spacer As Boolean = False)
        Try
        ...
        Catch ex As Exception
            Error_Handler(ex, "Activity Handler: " & message)
        End Try
    End Sub
        ...
        Try
            ...
            If AutoUpdate = True Then
                If My.Computer.FileSystem.FileExists((Application.StartupPath & "\AutoUpdate.exe").Replace("\\", "\")) = True Then
                    ...
                    Activity_Handler("AutoUpdate Requested")
                End If
            End If
            Activity_Handler("SAN Access Closed at " & Format(Now(), "hh:mm:ss dd/MM/yyyy"))
        Catch ex As Exception
            Error_Handler(ex, "Application Close")
        End Try
    End Sub

    Private Sub RequestFTPPasswordUpdate(Optional ByVal runtests As Boolean = True, Optional ByVal displayalert As Boolean = True)
        Try
            Activity_Handler("Called: Request FTP Password Update")
            If displayalert = True Then
                MsgBox("Password Synchronisation Error Encountered: It would seem that you have recently changed your password and there hasn't been enough time for the SAN Management System to synchronise the passwords on the system yet. This process may take up to 15 minutes to complete, but please feel free to use the HTTP Upload Component located on the My Files page.", MsgBoxStyle.Exclamation, "Password Synchronisation Error")
            End If


            If runtests = True Then

                'get correct password for FTP server. The database has a history of 4 passwords stored
                Dim passwordcorrect As Boolean = False
                ...
                For counter As Integer = 1 To 4
                    ...
                    Select Case counter
                        Case 1
                            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/getclearpass.php?variable=" & "SAN_ClearPass" & "&number=" & currentusername, tempfile, "", "", False, 100000, True)
                            ...
                            Dim request As FtpWebRequest = WebRequest.Create(New Uri("ftp://comsan.uct.ac.za:21"))
                            ...
                            Catch ex As Exception
                                'MsgBox(ex.Message & " (" & currentpassword & ")")
                            End Try
                            response = Nothing
                            request = Nothing
                        Case 2
                            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/getclearpass.php?variable=" & "SAN_PreviousClearPass" & "&number=" & currentusername, tempfile, "", "", False, 100000, True)
                            If My.Computer.FileSystem.FileExists(tempfile) = True Then
                                Dim reader As StreamReader = New StreamReader(tempfile)
                                If reader.Peek <> -1 Then
                                    currentpassword = reader.ReadLine()
                            ...
                            request = Nothing
                        Case 3
                            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/getclearpass.php?variable=" & "SAN_PreviousClearPass2" & "&number=" & currentusername, tempfile, "", "", False, 100000, True)
                            If My.Computer.FileSystem.FileExists(tempfile) = True Then
                                Dim reader As StreamReader = New StreamReader(tempfile)
                                If reader.Peek <> -1 Then
                                    currentpassword = reader.ReadLine()
                            ...
                            request = Nothing
                        Case 4
                            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/getclearpass.php?variable=" & "SAN_PreviousClearPass3" & "&number=" & currentusername, tempfile, "", "", False, 100000, True)
                            If My.Computer.FileSystem.FileExists(tempfile) = True Then
                                Dim reader As StreamReader = New StreamReader(tempfile)
                                If reader.Peek <> -1 Then
                                    currentpassword = reader.ReadLine()
                ...
                If passwordcorrect = False Then
                    'run server update request
                End If
            End If

            currentpasswordmasked = ""
            If currentpassword.Length > 3 Then
                For i As Integer = 1 To currentpassword.Length
                    currentpasswordmasked = currentpasswordmasked & "*"
                Next
                currentpasswordmasked = currentpasswordmasked.Remove(0, 2)
                currentpasswordmasked = currentpassword.Substring(0, 1) & currentpasswordmasked & currentpassword.Substring(currentpassword.Length - 1, 1)
            Else
                If currentpassword.Length > 0 Then
                    For i As Integer = 1 To currentpassword.Length
                        currentpasswordmasked = currentpasswordmasked & "*"
                    Next
                End If
            ...
            'run server update request
        Catch ex As Exception
            Error_Handler(ex, "RequestFTPPasswordUpdate")
        End Try
    End Sub

    Private Sub LoadSettings()
        Try
            Activity_Handler("Called: Load Settings")
    ...
    Private Sub SaveSettings()
        Try
            Activity_Handler("Call: Save Settings")
            Dim configfile As String = (Application.StartupPath & "\config.sav").Replace("\\", "\")
    ...
    Public Function ReturnRegKeyValue(ByVal MainKey As String, ByVal RequestedKey As String, ByVal Value As String) As String
        Activity_Handler("Call: Return Registry Key Value")
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Activity_Handler("SAN Access loaded at " & Format(Now(), "hh:mm:ss dd/MM/yyyy"), True)
            ...
            Else
                ...
                WebBrowser1.Navigate("http://comsan.uct.ac.za/Web/SAN Access/setusername.asp?str_currentuser=" & currentusername & "&str_currentcontext=" & currentstaffswitch)
    ...
    Private Sub HelpToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles HelpToolStripMenuItem.Click
        Try
            Activity_Handler("Call: Help Dialog Requested")
    ...
    Private Sub LoginToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LoginToolStripMenuItem.Click
        Try
            Activity_Handler("Call: Login Page Requested")
            WebBrowser1.Navigate("http://comsan.uct.ac.za/Web/SAN Access/logout.php")
        Catch ex As Exception
    ...
    Private Sub CurrentSessionVariablesToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CurrentSessionVariablesToolStripMenuItem.Click
        Try
            Activity_Handler("Call: Current Session Variables Dialog Requested")
    ...
    Private Sub AboutSANAccessToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AboutSANAccessToolStripMenuItem.Click
        Try
            Activity_Handler("Call: About Dialog Requested")
    ...
    Private Sub AutoUpdateToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AutoUpdateToolStripMenuItem.Click
        Try
            Activity_Handler("Call: AutoUpdate Requested")
            AutoUpdate = True
            Me.Close()
        Catch ex As Exception
    ...
    Private Sub BackgroundWorker1_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker1.DoWork
        Try
            ...
            If WebBrowser1.Url.ToString.ToLower.StartsWith("http://comsan.uct.ac.za/web/san access/my_san_file_list.php") Then
                ...
                End If
            End If
            RequestFTPPasswordUpdate(True, False)
        Catch ex As Exception
    ...
    Private Sub GetCurrentStaffSwitch(Optional ByVal force As Boolean = False)
        Try
            If My.Computer.FileSystem.FileExists(tempfile) = False Or force = True Then
                If currentusername.Length > 0 Then
                    DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/getaccountswitch.php?number=" & currentusername, tempfile, "", "", False, 100000, True)
                    If My.Computer.FileSystem.FileExists(tempfile) = True Then
                        Dim reader As StreamReader = New StreamReader(tempfile)
                        If reader.Peek <> -1 Then
                            currentstaffswitch = reader.ReadLine()
                        End If
                        reader.Close()
                        reader = Nothing
                    ...
                    End If
                End If
                If currentstaffswitch <> "Students" And currentstaffswitch <> "Staff" Then
                    currentstaffswitch = "Students"
                End If
            End If
        Catch ex As Exception
            Error_Handler(ex, "Get Current Staff Switch")
        End Try
    End Sub
    ...
    Private Sub runUploadInitialiser()
        Try
            Activity_Handler("Call: Upload Initialiser")
            Timer1.Stop()
    ...
    Private Sub Uploader(ByVal MyFiles() As String)
        Try
            Activity_Handler("Call: Uploader")
    ...
    Private Sub controls_enabler(ByVal enable As Boolean, Optional ByVal whichfunction As String = "")
        Try
            Activity_Handler("Call: Controls Enabler (" & enable & ")")
            Select Case enable
                Case True
                    MenuStrip1.Enabled = True
    ...
    Private Sub BackgroundWorker2_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker2.DoWork
        Try
            ...
            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/returnUsageStat.php?username=" & currentusername & "&filename=" & currentstaffswitch & "\" & currentusername, usagestatus, "", "", False, 100000, True)
            Dim lineread As String = ""
        ...
        Try
            ...
            If finfo.Exists = True Then
                Try
                    ...
                    DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/doesfileexist.php?filename=" & recursivecurrentuploadfolder.Replace("/", "\") & "\" & StripSpecialCharacters(finfo.Name), existstatus, "", "", False, 100000, True)
                    ...
                    If performUpload = True Then
                        ...
                        Try
                            ...
                            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/getMD5Checksum.php?filename=" & (recursivecurrentuploadfolder).Replace("/", "\") & "\" & StripSpecialCharacters(finfo.Name), md5status, "", "", False, 100000, True)
            ...
            If dinfo.Exists = True Then
                Try
                    ...
                    DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/doesfolderexist.php?filename=" & recursivecurrentuploadfolder.Replace("/", "\") & "\" & StripSpecialCharacters(dinfo.Name), existstatus, "", "", False, 100000, True)
                    If My.Computer.FileSystem.FileExists(existstatus) = True Then
                        Dim reader As StreamReader = New StreamReader(existstatus)
                        Dim lineread As String = reader.ReadLine().Trim
                        ' MsgBox(lineread)
    ...
    Private Function GetMD5Checksum(ByVal filePath As String) As String
        Activity_Handler("Call: Get MD5 Checksum")
    ...
    Private Function StripSpecialCharacters(ByVal filepath As String) As String
        Activity_Handler("Call: Strip Special Characters")
    ...
    Private Sub CancelUpload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CancelUpload.Click
        Try
            Activity_Handler("Call: Upload Cancelled")
    ...
    Private Sub CreateFolderButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CreateFolderButton.Click
        Try
            Activity_Handler("Call: Create Folder")
    ...
    Private Sub BackgroundWorker3_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker3.DoWork
        ...
        Try
            ...
            If targetupload.Length > 0 Then
                DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/doesfolderexist.php?filename=" & currentfolder.Replace("/", "\") & "\" & StripSpecialCharacters(targetupload), existstatus, "", "", False, 100000, True)
                If My.Computer.FileSystem.FileExists(existstatus) = True Then
                    Dim reader As StreamReader = New StreamReader(existstatus)
                    Dim lineread As String = reader.ReadLine().Trim
                    ' MsgBox(lineread)
    ...
    Private Sub DeleteAllFilesButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteAllFilesButton.Click
        Try
            Activity_Handler("Call: Delete All Files")
    ...
    Private Sub DeleteAllContentButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteAllContentButton.Click
        Try
            Activity_Handler("Call: Delete All Content")
                ...
                For Each filename As String In filearray
                    Try
                        ...
                        If filename.StartsWith("_directory_ ") = False Then
                            ...
                            request = Nothing
                        Else
                            DownloadFileRequested("http://comsan.uct.ac.za/Web/SAN Access/SAN_Access_FolderDelete.php?foldername=" & currentfolder.Replace("/", "\") & "\" & filename.Replace("_directory_ ", ""), existstatus, "", "", False, 100000, True)
                            If My.Computer.FileSystem.FileExists(existstatus) = True Then
                                My.Computer.FileSystem.DeleteFile(existstatus)
                            End If
                        End If
    ...
    Private Sub DeleteAllFoldersButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteAllFoldersButton.Click
        Try
            Activity_Handler("Call: Delete All Folders")
    ...
    Private Sub DownloadAllFilesButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DownloadAllFilesButton.Click
        Try
            Activity_Handler("Call: Download All Files")
            Dim UploadPath As String
            If WebBrowser1.Url.ToString.LastIndexOf("&") <> -1 Then
                UploadPath = WebBrowser1.Url.ToString.Remove(0, WebBrowser1.Url.ToString.IndexOf("=") + 1).Substring(0, WebBrowser1.Url.ToString.LastIndexOf("&"))
            Else
                ...
                Try
                    ...
                    If filename.StartsWith("_directory_ ") = False Then
                        ...
                        If My.Computer.FileSystem.FileExists(downloadtopath & "\" & filename) = True Then
                            If MsgBox(downloadtopath & "\" & filename & " already exists on the system. Do you wish to replace the existing file with this one?", MsgBoxStyle.Exclamation + MsgBoxStyle.YesNo, "File aready exists") = MsgBoxResult.Yes Then
                                DownloadFileRequested(filetodownloadpath, downloadtopath & "\" & filename, currentusername, currentpassword, True, 100000, True)
                            End If
                        Else
                            DownloadFileRequested(filetodownloadpath, downloadtopath & "\" & filename, currentusername, currentpassword, True, 100000, True)
                        End If
    ...
    Private Sub DownloadAllFoldersButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DownloadAllFoldersButton.Click
        Try
            Activity_Handler("Call: Download All Folders")
            Dim UploadPath As String
            If WebBrowser1.Url.ToString.LastIndexOf("&") <> -1 Then
                UploadPath = WebBrowser1.Url.ToString.Remove(0, WebBrowser1.Url.ToString.IndexOf("=") + 1).Substring(0, WebBrowser1.Url.ToString.LastIndexOf("&"))
            Else
    ...
    Private Sub DownloadAllContentButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DownloadAllContentButton.Click
        Try
            Activity_Handler("Call: Download All Content")
            Dim UploadPath As String
            If WebBrowser1.Url.ToString.LastIndexOf("&") <> -1 Then
                UploadPath = WebBrowser1.Url.ToString.Remove(0, WebBrowser1.Url.ToString.IndexOf("=") + 1).Substring(0, WebBrowser1.Url.ToString.LastIndexOf("&"))
            Else
    ...
    Private Sub recursivedownloader(ByVal inputtodownloadpath As String, ByVal downloadtopath As String)
        Try
            Activity_Handler("Call: Recursive Downloader")
    ...
    Private Sub SingleFileDownload(ByVal lastURL As String)
        Try
            Activity_Handler("Call: Single File Download")
            If currentusername.IndexOf("\") <> -1 Then
                currentusername = currentusername.Substring(0, currentusername.IndexOf("\"))
            End If
            Dim UploadPath As String
            ...
            If SaveFileDialog1.ShowDialog = Windows.Forms.DialogResult.OK Then
                ...
                Try
                    DownloadFileRequested(UploadPath, downloadtopath, currentusername, currentpassword, True, 100000, True)
                Catch ex As Exception
    ...
    Private Sub DownloadFileRequested(ByVal address As String, ByVal destinationFileName As String, ByVal userName As String, ByVal password As String, ByVal showUI As Boolean, ByVal connectionTimeout As Integer, ByVal overwrite As Boolean)
        Try
            My.Computer.Network.DownloadFile(address, destinationFileName, userName, password, showUI, connectionTimeout, overwrite)
            Activity_Handler("File Download: " & address.Replace("ftp://comsan.uct.ac.za/", "SAN_FROOT/").Replace("http://comsan.uct.ac.za/Web/SAN Access/", "SAN_ROOT/") & " - " & destinationFileName & " (" & currentusername & ")(" & currentpasswordmasked & ")")
        Catch ex As Exception
            Error_Handler(ex, "DownloadFileRequested: " & address & " - " & destinationFileName)
        End Try
    End Sub
    ...
    Private Sub WebBrowser1_Navigated(ByVal sender As System.Object, ByVal e As System.Windows.Forms.WebBrowserNavigatedEventArgs) Handles WebBrowser1.Navigated
        Try
            ...
            Activity_Handler("Browser: " & WebBrowser1.Url.ToString.Replace("http://comsan.uct.ac.za/Web/SAN Access/", "SAN_ROOT/"))
        Catch ex As Exception
            Error_Handler(ex, "WebBrowser1_Navigated: " & WebBrowser1.Url.ToString)
        End Try
    End Sub
